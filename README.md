...
<table>
	...
	<tr>
		<td><a href="https://raw.githubusercontent.com/equinix/agent-factory/refs/heads/main/agent_factory_schema/equinix/fabric/v1/on_event/port/port-bandwidth-usage-alert.md">Port Bandwidth Usage Alert Agent<br>[port-bandwidth-usage-alert.md]</a></td>
		<td>An Equinix agent that monitors Fabric port bandwidth usage and notifies a user when a bandwidth alert is raised on a port.
This agent only executes once per event.</td>
		<td>- Monitor real-time network event streams for port bandwidth usage<br>- Detect and validate bandwidth alert rules raised on a port<br>- Resolve the affected port's details (name, metro, account)<br>- Send notifications for critical bandwidth alerts</td>
		<td>This skill can use the following tools:
