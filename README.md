...
*   **`send_email_notification`**: Sends an email notification given an email address and email body.</td>
	...
	<tr>
		<td><a href="https://raw.githubusercontent.com/equinix/agent-factory/refs/heads/main/agent_factory_schema/equinix/fabric/v1/run_once/connection-event-metric.md">List Active Connections and Retrieve CloudEvents and Bandwidth Metrics<br>[connection-event-metric.md]</a></td>
		<td>This skill lists 3 active connections in user's project and retrieves their CloudEvents and bandwidth metrics for monitoring and analysis.
This agent can only run once.</td>
		<td>- List active connections (up to 3<br>- Retrieve CloudEvents for each connection<br>- Fetch bandwidth metrics for each connection<br>- Log all actions and results</td>
		<td>This skill can use the following tools:
...
* **`get_metrics`**: Fetches bandwidth tx ans rx  metrics for a specific connection UUID</td>
		<td>preview
	</tr>
</table>
...
<table>
	...
	<tr>
		...
		<td>An Equinix agent that automatically boosts connection bandwidth to mitigate traffic-induced packet loss.</td>
