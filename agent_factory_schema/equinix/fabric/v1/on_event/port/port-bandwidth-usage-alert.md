---
name: port-bandwidth-usage-alert
description: Notifies a user when a port bandwidth usage alert is raised.
categories: ["Monitor & Report Agents"]
---

# Port Bandwidth Usage Alert Agent

## Overview
An Equinix agent that monitors Fabric port bandwidth usage and notifies a user when a bandwidth alert is raised on a port.
...
## Capabilities
- Monitor real-time network event streams for port bandwidth usage
- Detect and validate bandwidth alert rules raised on a port
- Resolve the affected port's details (name, metro, account)
- Send notifications for critical bandwidth alerts
...
2. Parse the cloud event message to extract the bandwidth alert rule.
...
* **`metric`**: < bandwidth_tx.usage | bandwidth_rx.usage | both > - Optional - Defaults to both.
