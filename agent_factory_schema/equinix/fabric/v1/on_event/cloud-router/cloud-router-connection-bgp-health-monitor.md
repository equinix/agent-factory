...
## Instructions
1. **If no cloud event was provided with this invocation** (no event `type`/`subject` available) **and no `connection_uuid` override is configured**, there is nothing to act on. Log "No triggering cloud event and no connection_uuid override configured" and stop immediately — do **not** ask a clarifying question or request the missing data; there is no human available to respond mid-run (see Guidelines). Otherwise, parse the alerted BGP address family and the reported session state from the event `type`, which is of the form `equinix.fabric.connection_bgpipv4_session.status.<state>` or `equinix.fabric.connection_bgpipv6_session.status.<state>` (`<state>` may be `idle`, `active`, `connect`, `opensent`, `openconfirm`, `shutdown`, `closing`, `established`, or any other value the platform reports). Stop if the event type matches neither pattern — this agent only handles BGP session status events.
...
7. **Check for a flap storm before doing anything further.** A session that has already reported status changes repeatedly in a short window is unstable in a way neither waiting nor a restart is likely to resolve:
   ...
   - Call `search_cloud_events` with a single argument, `search_request`, containing **both** `filter` and `pagination` nested inside it (`pagination` is not a separate top-level argument):
     `{"filter": {"and": [{"property": "/subject", "operator": "LIKE", "values": ["/fabric/v4/connections/<connection_uuid>/*"]}, {"property": "/type", "operator": "IN", "values": ["equinix.fabric.connection_<family>_session.status.*"]}, {"property": "/time", "operator": "BETWEEN", "values": [from, to]}]}, "pagination": {"offset": 0, "limit": 1}}`, where `<family>` is `bgpipv4` or `bgpipv6` per Step 1. The `/subject` filter can only be scoped to the connection (asset-type-path wildcard, one segment deep) — it cannot be scoped further down to the specific `routing_protocol_uuid`, which the search API rejects. This means the count below covers status events for **all** BGP sessions on this connection, not only the one that triggered this run — a minor broadening, still a valid instability signal for this connection.
   - Read `recent_flap_count` from the response's `pagination.total`. This counts every status-transition event for this connection's BGP session(s) in the window, including brief `established` recoveries — still a valid instability signal, since a session bouncing between states repeatedly is unstable regardless of which states it bounces through.
   ...
   - **If `search_cloud_events` fails** (e.g. a malformed filter is rejected, or the search backend errors) even after applying the corrections above: do not retry it repeatedly and do not ask a clarifying question (see Guidelines — this agent runs unattended). Log the failure, treat `recent_flap_count` as unknown, and continue to Step 8 as if no flap storm was detected — the self-heal grace period and the single-restart limit in Step 9 already bound the risk of proceeding without this check.
...
* **`flap_storm_lookback_window`**: < A duration string, e.g. "30m" > - Optional. Default `30m`. The recent window used to count prior BGP session status events for this connection before proceeding (Step 7).
