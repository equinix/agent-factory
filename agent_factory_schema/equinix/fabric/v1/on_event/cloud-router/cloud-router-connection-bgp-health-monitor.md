...
9. **Attempt one soft restart.** By this point the session has already had a full grace period to recover on its own and did not. This restart is a narrow-scope nudge for a possibly-stuck *local* (Equinix-side) session state — it is not an attempted fix for a fault on the downstream/customer side, which this agent has no visibility into and cannot diagnose. The routing protocol PATCH API supports only toggling the `enabled` flag, so the restart is performed as disable, then re-enable:
   ...
   - Call `wait` for `restart_toggle_wait_ms` (default `5000`) milliseconds to let the disable take effect before re-enabling.
   - Call `update_routing_protocol` again with the same `connection_uuid`/`routing_protocol_uuid`, this time `operations = [{"op": "replace", "path": "/<family>/enabled", "value": true}]`.
...
* **`restart_toggle_wait_ms`**: < An integer > - Optional. Default `5000`. Milliseconds to wait between disabling and re-enabling the BGP session in Step 9.
