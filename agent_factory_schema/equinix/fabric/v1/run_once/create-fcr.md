...
*   **`get_router_package`**: Retrieves details of a specific router package (e.g., STANDARD, PREMIUM, LAB) including supported metros and bandwidth tiers.
...
## Instructions
1. If the user provides a `package_code`, use `get_router_package` to validate that the package exists and retrieve its details (supported metros, bandwidth limits, etc.). If no `package_code` is provided, default to `"STANDARD"`.
...
3. Use `create_router` to create the cloud router with the following payload structure:
    ```json
    {
      "type": "XF_ROUTER",
      "name": "<name>",
...
*   **Package Validation**: Validate the router package exists and supports the target metro before attempting creation.
*   **Error Handling**: If parameters are invalid, the package is unavailable, or the creation fails, log errors clearly and stop the process. Do not retry automatically.
...
* **`account_number`**: < Equinix account number > — Required — The billing account number to associate with the router.
* **`project_id`**: < Project ID > — Required — The Equinix project to associate the router with.
* **`term_length`**: < Contract term length, e.g. `"1"`, `"12"`, `"24"`, `"36"` > — Optional — Order term length in months. Defaults to `"1"`.
