...
## Overview
An Equinix agent that audits all Fabric Ports to detect ports that are not attached to any stream
...
- Call `attach_stream_asset` with the port UUID, the configured `stream_uuid`, and `"metrics_enabled": false`.
