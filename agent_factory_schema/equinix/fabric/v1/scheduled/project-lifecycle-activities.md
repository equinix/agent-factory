...
## Overview
This definition sets up and activates an Equinix Fabric CloudEvent Intelligence Agent that analyzes all cloud events within a given project over the last 24 hours and delivers a clear, plain-English operational health summary via email every day. The report is designed to be read in under two minutes — telling the customer exactly what happened, whether anything needs attention, and what action (if any) to take. This agent is triggered once every day.
...
## Capabilities
- Search all cloud events for a project over the last 24 hours
...
- Send the summary as an email notification (with a PDF attachment) to the designated recipient
...
1b. Set the reporting window as follows:
