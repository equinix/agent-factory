...
- **Routing Protocol & BGP Health**: Include only if BGP/routing events exist — otherwise omit entirely. Reference sessions using both the connection name/full UUID and routing protocol full UUID. Per session: 0–1 transitions = routine, 2–3 = transient but recovered, 4+ = flapping. Always state final observed session state.
